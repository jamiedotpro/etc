...
# monitor='loss'    monitor는 'loss', 'val_loss' 쓰느게 좋음
# monitor = 'val_loss' validation 데이터가 있다면 그걸로해도 됨
# monitor = 'val_acc'
# early_stopping = EarlyStopping(monitor='loss', patience=50, mode='auto')
# early_stopping = EarlyStopping(monitor='val_loss', patience=10, mode='auto')
early_stopping = EarlyStopping(monitor='val_acc', patience=10, mode='auto')
model.fit(x_train, y_train, epochs=10000, batch_size=1, validation_data=(x_val, y_val), callbacks=[early_stopping])   # validation_data 훈련하면서 검증까지 함. 추가하면 훈련이 더 잘됨
