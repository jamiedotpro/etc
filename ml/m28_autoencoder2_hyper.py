...
def build_network(keep_prob=0.5, optimizer='adam', layer_cnt=1, node1=50):
    ...
    prediction = Dense(784, activation='sigmoid', name='output')(x)
...
def create_hyperparameters():
    ...
    return{'batch_size':batches, 'optimizer':optimizers,
            'keep_prob':dropout, 'layer_cnt':layer_cnt, 'node1':node1}
...
model = build_network(model_dic['keep_prob'], model_dic['optimizer'],
                        model_dic['layer_cnt'], model_dic['node1'])

from keras.callbacks import EarlyStopping
early_stopping = EarlyStopping(monitor='loss', patience=30, mode='auto')
history = model.fit(x_train, x_train, epochs=100,
                    batch_size=model_dic['batch_size'],
